...
def add_message(
    ...
    cursor.execute(
        ...
        "(user_name, content, timestamp, file_name, file_data) "
        "VALUES (?, ?, ?, ?, ?)",
        (
            user_name,
            content,
...
def get_all_messages():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, user_name, content, timestamp, file_name "
...
def get_file(message_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT file_name, file_data FROM messages WHERE id = ?", (message_id,)
    )
    row = cursor.fetchone()
    conn.close()
    return row
