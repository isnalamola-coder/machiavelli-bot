...
class MilitaryResolver:
    ...
    def _is_defensible_city(self, city_type: str | None) -> bool:
        """Delegate defensible-city policy to the active scenario."""
        scenario = self.game.scenario
        if scenario is None:
            return city_type in {"fortified", "fortress"}
        return scenario.is_defensible_city(city_type)
        ...
        if order.order_type == "A":
            ...
            valid = (
                ...
                or (
                    key.unit_type == "A"
                    and target == "G"
                    and self._is_defensible_city(location.city)
                )
                or (
                    key.unit_type == "F"
                    and target == "G"
    ...
    def _validate_final_collections(
        ...
        for collections in players.values():
            for unit_type, locations in collections.items():
                for location in locations:
                    ...
                    if unit_type == "G":
                        province = self.map.provinces.get(location)
                        if province is None or not self._is_defensible_city(
                            province.city
                        ):
                            raise MilitaryResolutionError(
                                "Guarnición final fuera de plaza defendible"
                            )
                    occupied = (
        ...
        for location in independent:
            ...
            if province is None or not self._is_defensible_city(province.city):
                raise MilitaryResolutionError(
                    "Guarnición independiente final fuera de plaza defendible"
                )
    ...
    def _apply_dislodgement_decisions(
        ...
        for key in sorted(decisions, key=_key_sort):
            ...
            if unit_type == "G":
                province = self.map.provinces.get(destination)
                if province is None or not self._is_defensible_city(province.city):
                    raise MilitaryResolutionError(
                        "Retirada de guarnición fuera de plaza defendible"
                    )
